...
brainstorming_agent = Agent(
    ...
    instructions=[
        ...
        "",
        "<MANDATE>",
        "Your response is NOT complete until you have PHYSICALLY WRITTEN the SPEC.md file to disk.",
        "If SPEC.md does not exist on disk after your turn, you have FAILED. Retry immediately.",
        "</MANDATE>",
        "",
        "<FILETOOLS-API>",
        "Use SuperpowersTools.write_spec(contents='...') to write SPEC.md.",
        "This is the MOST RELIABLE method. It only needs the file contents.",
        "Example: SuperpowersTools.write_spec(contents='# Specification\\n...')",
        "</FILETOOLS-API>",
        "",
        "Mandatory Checklist:",
        ...
        "7. Write the complete specification using SuperpowersTools.write_spec(contents=...)",
...
planning_agent = Agent(
    ...
    instructions=[
        ...
        "",
        "<MANDATE>",
        "Your response is NOT complete until you have PHYSICALLY WRITTEN the PLAN.md file to disk.",
        "If PLAN.md does not exist on disk after your turn, you have FAILED. Retry immediately.",
        "</MANDATE>",
        "",
        "<FILETOOLS-API>",
        "Use SuperpowersTools.write_plan(contents='...') to write PLAN.md.",
        "This is the MOST RELIABLE method. It only needs the file contents.",
        "Example: SuperpowersTools.write_plan(contents='# Plan\\n...')",
        "</FILETOOLS-API>",
        "",
        "1. Read SPEC.md from disk using SuperpowersTools.read_file(file_name='SPEC.md'). If it does not exist, report failure and STOP.",
        ...
        "5. Write the complete PLAN.md using SuperpowersTools.write_plan(contents=...)",
...
tdd_developer = Agent(
    ...
    instructions=[
        ...
        "",
        "<MANDATE>",
        "Your response is NOT complete until you have PHYSICALLY WRITTEN code files and test files to disk.",
        ...
        "",
        "<FILETOOLS-API>",
        "For SPEC, PLAN, REVIEW: use single-parameter helpers:",
        "  SuperpowersTools.write_spec(contents='...')",
        "  SuperpowersTools.write_plan(contents='...')",
        "  SuperpowersTools.write_review(contents='...')",
        "For code and test files: use write_code_file or write_test_file:",
        "  SuperpowersTools.write_code_file(file_name='main.py', contents='...')",
        "  SuperpowersTools.write_test_file(file_name='tests/test_auth.py', contents='...')",
        "These are MORE RELIABLE than FileTools.save_file. Always use them for saving.",
        "</FILETOOLS-API>",
        "",
        "1. Read PLAN.md from disk using SuperpowersTools.read_file(file_name='PLAN.md'). If it does not exist, report failure and STOP.",
        "2. RED: Write ONE minimal failing test. Requirements:",
        "   - One behavior per test",
        "   - Clear name describing behavior",
        "   - Real code, not mocks",
        "   - Write the test to disk BEFORE running it using SuperpowersTools.write_test_file(file_name='tests/test_xxx.py', contents=...)",
        "3. VERIFY RED: Run the test suite using ShellTools and confirm it FAILS for the expected reason.",
        "4. GREEN: Write the SIMPLEST code that makes the test pass.",
        "   - Write production code to disk using SuperpowersTools.write_code_file(file_name='xxx.py', contents=...)",
        "5. VERIFY GREEN: Run the FULL test suite and confirm all tests pass.",
        "6. ONLY AFTER GREEN IS VERIFIED: Use SuperpowersTools.evaluate_gate('tdd-gate') to confirm progress.",
...
review_agent = Agent(
    ...
    instructions=[
        ...
        "",
        "<MANDATE>",
        "Your response is NOT complete until you have PHYSICALLY WRITTEN the REVIEW.md file to disk.",
        "If REVIEW.md does not exist on disk after your turn, you have FAILED. Retry immediately.",
        "</MANDATE>",
        "",
        "<FILETOOLS-API>",
        "Use SuperpowersTools.write_review(contents='...') to write REVIEW.md.",
        "This is the MOST RELIABLE method. It only needs the file contents.",
        "Example: SuperpowersTools.write_review(contents='# Review\\n...')",
        "</FILETOOLS-API>",
        "",
        "1. Read SPEC.md and PLAN.md from disk using SuperpowersTools.read_file(). If they do not exist, report failure and STOP.",
        ...
        "",
        "Review Output Format (write to REVIEW.md using SuperpowersTools.write_review(contents=...)):",
